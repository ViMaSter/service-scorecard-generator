...
func TestAzureVisualizerMatchesCSharpFixture(t *testing.T) {
	now := time.Date(2026, time.April, 3, 12, 0, 0, 0, time.UTC)
	outputDirectory := t.TempDir()
	v := visualizer.NewAzureWikiTableVisualizer(outputDirectory, func() time.Time { return now })
	...
	for _, expected := range []string{"# Service Scorecard for 2026-04-03", "# Hover over cells for details", "service", "service2", "Check", "DisqualifiedCheck", "\"ServiceScores\""} {
		if !strings.Contains(mainFile, expected) {
			t.Fatalf("expected azure output to contain %q", expected)
		}
	}
