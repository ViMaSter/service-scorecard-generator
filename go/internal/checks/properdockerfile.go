...
func (c *ProperDockerfile) Run(absolutePathToProjectFile string) []scorecard.Deduction {
	...
	if !strings.Contains(dockerfileContent, "dotnet build") && !strings.Contains(dockerfileContent, "dotnet publish") {
		return []scorecard.Deduction{scorecard.NewDeduction(100, "Dockerfile at %v does not contain 'dotnet build' or 'dotnet publish'", relPath(dockerfile))}
	}
	if !strings.Contains(dockerfileContent, "dotnet sonarscanner") {
		return []scorecard.Deduction{scorecard.NewDeduction(50, "Dockerfile at %v doesn't contain 'dotnet sonarscanner'", relPath(dockerfile))}
	}
	if strings.Contains(dockerfileContent, "--version") {
		return []scorecard.Deduction{scorecard.NewDeduction(10, "Dockerfile at %v contains pinned version that won't be caught by Renovate", relPath(dockerfile))}
	}
	return nil
}
