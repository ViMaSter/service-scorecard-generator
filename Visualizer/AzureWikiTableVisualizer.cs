...
internal class AzureWikiTableVisualizer
{
    ...
    public string ToMarkdown(Calculation.RunInfo runInfo)
    {
        ...
        return $"{generationInfo}{Environment.NewLine}{Environment.NewLine}<table style=\"height: 40vh\">{string.Join(Environment.NewLine, output.Prepend(headers).Prepend(groupData).Prepend(""))}</table>";
    }
