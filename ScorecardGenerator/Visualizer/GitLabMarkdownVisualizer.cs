...
public class GitLabMarkdownVisualizer : IVisualizer
{
    ...
    private void GenerateServiceOverview(RunInfo runInfo)
    {
        ...
        var checkNames = runInfo.Checks.Values.SelectMany(checksInGroup => checksInGroup).Select(check => check.Name).ToList();

        var headerColumns = string.Join(" | ", new[] { "Service" }.Concat(checkNames).Append("Average"));
        var separatorColumns = string.Join(" | ", Enumerable.Repeat("---", checkNames.Count + 2));
        ...
        var rows = runInfo.ServiceScores.Select(pair =>
        {
            ...
            return $"| {string.Join(" | ", checkCells.Prepend(serviceName).Append(average.ToString()))} |";
        });

        var table = string.Join(Environment.NewLine, new[]
        {
            $"| {headerColumns} |",
            $"| {separatorColumns} |"
        }.Concat(rows));
    ...
    private static TableContent FormatJustifiedScore(IList<BaseCheck.Deduction> checkValue, IList<BaseCheck.Deduction>? sevenDaysAgo)
    {
        ...
        var scoreContent = finalScore == null ? "n/a" : finalScore.ToString();
        return new TableContent($"<span title=\"{EscapeTitle(justifications)}\">{scoreContent}{deltaString}</span>", justifications);
    }
