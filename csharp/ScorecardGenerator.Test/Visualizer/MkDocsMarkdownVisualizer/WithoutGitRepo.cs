...
namespace ScorecardGenerator.Test.Visualizer.MkDocsMarkdownVisualizer;
...
public class WithoutGitRepo : TestWithNeighboringDirectoryFixture
{
    ...
    public void DeterministicallyRendersServiceInfo()
    {
        ...
        var visualizer = new ScorecardGenerator.Visualizer.MkDocsMarkdownVisualizer(logger, tempPath);
    ...
    private static void CompareFilesInsideDirectoriesWithNUnitAsserts(string absolutePathToDirectoryActual, string absolutePathToDirectoryExpected)
    {
        ...
        foreach (var actualFile in actualFiles)
        {
            ...
            Assert.That(ScorecardGenerator.Visualizer.MkDocsMarkdownVisualizer.RemoveDates(File.ReadAllText(actualFile.FullName)), Is.EqualTo(ScorecardGenerator.Visualizer.MkDocsMarkdownVisualizer.RemoveDates(File.ReadAllText(expectedFilesDictionary[actualFile.Name].FullName))));
        }
    }
}
