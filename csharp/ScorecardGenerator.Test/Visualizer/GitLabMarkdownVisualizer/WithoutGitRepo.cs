...
namespace ScorecardGenerator.Test.Visualizer.GitLabMarkdownVisualizer;
...
public class WithoutGitRepo : TestWithNeighboringDirectoryFixture
{
    ...
    public void DeterministicallyRendersServiceInfo()
    {
        ...
        var visualizer = new ScorecardGenerator.Visualizer.GitLabMarkdownVisualizer(logger, tempPath);
    ...
    private static void CompareFilesInsideDirectoriesWithNUnitAsserts(string absolutePathToDirectoryActual, string absolutePathToDirectoryExpected)
    {
        ...
        foreach (var actualFile in actualFiles)
        {
            ...
            Assert.That(ScorecardGenerator.Visualizer.GitLabMarkdownVisualizer.RemoveDates(File.ReadAllText(actualFile.FullName)), Is.EqualTo(ScorecardGenerator.Visualizer.GitLabMarkdownVisualizer.RemoveDates(File.ReadAllText(expectedFilesDictionary[actualFile.Name].FullName))));
        }
    }
}
